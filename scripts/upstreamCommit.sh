...
if [ -n "$paperHash" ]; then
    ...
    paper=$(getCommits "PaperMC/Paper" "$paperHash" $(echo $newHash | grep . -q && echo $newHash || echo "dev/26.2")) # Update this on every version update
...
if [ -n "$purpurHash" ]; then
    purpur=$(getCommits "PurpurMC/Purpur" "$purpurHash" "ver/26.2") # Update this on every version update
